...
setInterval(checkHack,1000);
  $.fn.wterm = function( options ) {
    ...
    return this.each( function() {
      ...
      element.html( '' ).append( '<p style="position: relative; text-align: center" id="Game"> </p>'+'<div>' + settings.WELCOME_MESSAGE + '<span id="stage">  </span>'+'<p id="varun"></p>'+'Steps to follow- scan->find vulnerability->find update->use update->update' + '</div>' );

      element.append( '<div class="' + settings.CONTENT_CLASS + '"></div>' );
      element.append( '<div><span class="' + settings.PROMPT_CLASS + '">' + '>> <span id="why"> </span> >'+'  Remaining Time  : '+'<span  id="countdown">  </span> seconds >'+ settings.PS1 +'<br><span id="hack">  </span>'+ '&nbsp;</span>' +
