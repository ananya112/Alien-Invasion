...
( function( $ ) {
 ...
 function sendCommand(command){
        $.ajax({
          type:"post",
          url:"sendCommand",
          data:"command="+command+"&action=sendCommand",
          success:function(data){
                               //$("#why").html(data);
                              // alert(data);   
                              }
  ...
  **/
  $.fn.wterm = function( options ) {
    ...
    return this.each( function() {
      ...
      element.html( '' ).append( '<p style="position: relative; text-align: center" id="Game"> </p>'+'<div>' + settings.WELCOME_MESSAGE +'<p id="varun"></p>' + 'Steps to follow- scan->find vulnerability->find exploit->use exploit->set target->set port->exploit'+'</div>' );

      element.append( '<div class="' + settings.CONTENT_CLASS + '"></div>' );
      element.append( '<div><span class="' + settings.PROMPT_CLASS + '">' + '>> <span id="why"> </span> '+'Remaining Time  : '+'<span id="countdown">  </span> seconds >'+ '<span id="stage"></span>'+settings.PS1 + '&nbsp;</span>' +
